...
const [leftShow, setLeftShow] = useState<boolean>()
const [rightShow, setRightShow] = useState<boolean>()

const scrollRef = useRef<HTMLDivElement>(null)
const scroll= (direction : "left" | "right") => {
    if(!scrollRef.current) return
    const scrollAmnt = direction === "left" ? -300 : 300
    scrollRef.current.scrollBy({left:scrollAmnt, behavior:"smooth"})
}
...
// scrollWidth = Total scroll distance
//scrollLeft = in the left how much scroll done
// clientWidth = The scroll distance visible to client

// means when scrollLeft === scrollWidth --> extreme right
// when scrollLeft === 0 --> extreme left

setLeftShow(scrollLeft > 0)
setRightShow((scrollLeft+clientWidth) < scrollWidth - 5)
}
...
return ()=> clearInterval(scrollInt)
  ...
  return (
    ...
    >
      ...
      {leftShow &&
      <button className='absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg hover:bg-green-100 rounded-full w-10 h-10 flex items-center justify-center transition-all cursor-pointer'
      onClick={() => scroll("left")}
...
${cat.color} shadow-md hover:shadow-lg transition-all cursor-pointer`}
                >
                    <div className='flex flex-col items-center justify-center p-5'>
                        <Icon className='w-10 h-10 text-green-700 mb-3'/>
                        <p className='text-center text-sm md:text-base font-medium text-gray-700'>{cat.name}</p>
      ...
      {rightShow &&
      <button 
      className='absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg hover:bg-green-100 rounded-full w-10 h-10 flex items-center justify-center transition-all cursor-pointer'
      onClick={() => scroll("right")}
      ><ChevronRight className='w-6 h-6 text-green-700'/></button>}
    </motion.div>
  )
}
